...
type ManagePageActions = 'Detail' | 'Edit' | 'Volunteers';
...
export const ManageActivitiePage = () => {
  ...
  const {
    ...
    activitiesOwn,
    activitieDetail,
  } = useSelector((store: RootState) => store.activitiesSlice);

  const [isOwn, setIsOwn] = useState(false);
  const [manageActions, setManageActions] = useState<ManagePageActions>('Detail');

  const getActivitieDetailByParamId = () => {
    const activitieOwned = activitiesOwn.filter((activitie: any) => activitie.id === id);
    if (
    ...
    const activitieViewInfo = activities.filter((activitie: any) => activitie.id === id);
    dispatch(setActivitieDetail({ ...activitieViewInfo[0], volunteers: [] }));
  };

  const handleSignInClick = (
    type: ManagePageActions,
  ) => {
    ...
    <>
      ...
      }
      {
        ...
        && Object.keys(activitieDetail).length > 0 && (
          <ManageActivitieForm
            activitie={activitieDetail as Activitie}
